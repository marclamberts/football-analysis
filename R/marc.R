      ...
      filenames <- list.files("~/Python/LIV", pattern="*.json", full.names=TRUE)
      #filenames <- "~/Downloads/KKD/2023-04-17_LIV Ajax II - PSV Eindhoven II.json"
      all_games <- data.frame()
      colnames(all_games)
      #path to the csv with matches. Important for the team names (home and away)
      matches <- read.csv("/Users/marclambertes/Python/Schedules/LIV Matches.csv") %>%
        ...
        #mutate(TeamId= 
         #        stringi::stri_replace_all_regex(TeamId,
          #                                       pattern=c('SV Roda', 'SC ', 'Eindhoven II','Alkmaar Zaanstreek','LIV ', 'HFC ','BV '),
      ...
      for(i in 1:length(filenames)){
        ...
        write.csv(Wedstrijd, glue::glue('~/Python/LIV/xgCSV/{gsub(".json","",str_split_fixed(filenames[i], n=6,pattern = "/")[6])}.csv'))
        #######
      ...
      excel_file <- "LIV Expected Points.xlsx"  # Replace with your desired file path
      write.xlsx(df, excel_file)
      
  
      excel_file <- "xT.xlsx"  # Replace with your desired file path
      write.xlsx(xT, excel_file)
      
      excel_file <- "xA.xlsx"  # Replace with your desired file path
      write.xlsx(lijstMinuten, excel_file)
