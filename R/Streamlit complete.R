...
# Read competition and season pairs from Excel
# Excel file must have columns: competition_id, season_id
comp_season_df <- read.xlsx("combined_competitions.xlsx")  # <-- CHANGE filename as needed
...
# Store all matches
all_matches_list <- align_columns(all_matches_list)
all_matches_list <- list()

for (i in seq_len(nrow(comp_season_df))) {
  comp_id <- comp_season_df$competition_id[i]
  season_id <- comp_season_df$season_id[i]
